...
{
  gsize i, size = 0;
  g_auto(GStrv) hidden_devs = NULL;
  g_auto(GStrv) old_hidden_devs = NULL;
  g_auto(GStrv) old_enumerables = NULL;
  g_auto(GStrv) enumerables = g_key_file_get_string_list (metadata, FLATPAK_METADATA_GROUP_USB_DEVICES, FLATPAK_METADATA_KEY_USB_ENUMERABLE_DEVICES,
                                                          &size, NULL);

  if (old_metadata)
    old_enumerables = g_key_file_get_string_list (old_metadata, FLATPAK_METADATA_GROUP_USB_DEVICES, FLATPAK_METADATA_KEY_USB_ENUMERABLE_DEVICES,
                                                  NULL, NULL);
  ...
  hidden_devs = g_key_file_get_string_list (metadata, FLATPAK_METADATA_GROUP_USB_DEVICES, FLATPAK_METADATA_KEY_USB_HIDDEN_DEVICES,
                                            &size, NULL);

  if (old_metadata)
    old_hidden_devs = g_key_file_get_string_list (old_metadata, FLATPAK_METADATA_GROUP_USB_DEVICES, FLATPAK_METADATA_KEY_USB_HIDDEN_DEVICES,
                                                  NULL, NULL);

  for (i = 0; i < size; i++)
    {
      const char *hidden = hidden_devs[i];
      if (old_hidden_devs == NULL || !g_strv_contains ((const char * const *) old_hidden_devs, hidden))
