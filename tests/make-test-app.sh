...
if [ "$EXTRA" = "EXTENSIONS" ]; then
  flatpak build-export --no-update-summary --runtime ${collection_args} ${GPGARGS-} ${SIGNARGS-} ${EXPORT_ARGS-} ${REPO} ${DIR} v2
else
  flatpak build-export --no-update-summary --runtime ${collection_args} ${GPGARGS-} ${SIGNARGS-} ${EXPORT_ARGS-} ${REPO} ${DIR} v1
fi
