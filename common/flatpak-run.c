...
#ifdef ENABLE_XCB
G_DEFINE_AUTOPTR_CLEANUP_FUNC(xcb_connection_t, xcb_disconnect);

static char *flatpak_run_get_pulseaudio_server_x11(void) {
  g_autoptr(xcb_connection_t) connection = xcb_connect (NULL, NULL);
  xcb_screen_iterator_t iter;
  const xcb_setup_t *s;
  const char *property = "PULSE_SERVER";
  g_autofree xcb_intern_atom_reply_t *reply = NULL;
  xcb_get_property_cookie_t cookie;
  g_autofree xcb_get_property_reply_t *prop = NULL;
  size_t len;

  if (xcb_connection_has_error(connection))
    return NULL;

  s = xcb_get_setup(connection);
  if (!s)
    return NULL;
  ...
  reply = xcb_intern_atom_reply (connection,
                                 xcb_intern_atom(connection, 0, strlen (property), property),
                                 NULL);

  cookie = xcb_get_property (connection, 0, screen->root, reply->atom, XCB_ATOM_STRING, 0, 256);
  prop = xcb_get_property_reply(connection, cookie, NULL);
  ...
  return g_strndup(xcb_get_property_value(prop), len);
}
#endif
...
static char *flatpak_run_get_pulseaudio_server(void) {
  ...
  {
    char *pulse_server;
    pulse_server = flatpak_run_get_pulseaudio_server_x11();
    if (pulse_server) {
      g_debug("Found pulseaudio socket from X11: %s", pulse_server);
      return pulse_server;
    }
  }
#endif
...
static char* flatpak_run_parse_pulse_server(const char *value) {
  ...
  for (i = 0; servers[i] != NULL; ++i) {
    ...
    if (flatpak_has_path_prefix (server, "{")) {
