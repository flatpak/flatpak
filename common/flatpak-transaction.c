...
op_get_runtime_ref (FlatpakTransactionOperation *op)
{
  ...
  gboolean is_app = flatpak_decomposed_is_app (op->ref);
  gboolean is_extension = !is_app && g_key_file_has_group (op->resolved_metakey, "ExtensionOf");
  ...
  if (is_app)
    {
      g_autofree char *runtime_pref = g_key_file_get_string (op->resolved_metakey, "Application", "runtime", NULL);
      if (runtime_pref)
        decomposed = flatpak_decomposed_new_from_pref (FLATPAK_KINDS_RUNTIME, runtime_pref, &my_error);
    }
  else if (is_extension && g_key_file_get_boolean (op->resolved_metakey, "ExtensionOf", "RequireRef", NULL))
    {
      /* The extension requires its target to be installed */
      g_autofree char *app_ref = g_key_file_get_string (op->resolved_metakey, "ExtensionOf", "ref", NULL);
      decomposed = flatpak_decomposed_new_from_ref (app_ref, &my_error);
    }
  else if (is_extension && g_key_file_has_group (op->resolved_metakey, "Extra Data") &&
           !g_key_file_get_boolean (op->resolved_metakey, "Extra Data", "NoRuntime", NULL))
    {
      /* If the extension has extra data (and doesn't define NoRuntime) it needs a runtime
         so it can run the apply-extra script. */
      g_autofree char *runtime_pref = g_key_file_get_string (op->resolved_metakey, "ExtensionOf", "runtime", NULL);
      if (runtime_pref)
        decomposed = flatpak_decomposed_new_from_pref (FLATPAK_KINDS_RUNTIME, runtime_pref, &my_error);
    }
