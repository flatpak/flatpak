...
{
  g_autofree char *no_os_report_file = NULL;
  g_autofree char *os_report_config = NULL;
  g_autofree char *os_id = NULL;
  g_autofree char *os_version = NULL;

  no_os_report_file = g_build_filename (get_config_dir_location (),
                                        "no-report-os-info", NULL);

  os_report_config = flatpak_dir_get_config (self, "report-os-info", NULL);

  if (g_strcmp0 (g_getenv ("FLATPAK_REPORT_OS_INFO"), "false") == 0 ||
      g_file_test (no_os_report_file, G_FILE_TEST_EXISTS) ||
      g_strcmp0 (os_report_config, "false") == 0)
    return NULL;
